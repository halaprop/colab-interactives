 ...
 * diagrams. Loaded as an ES module, in both the Vite harness and the
 * Colab shim, so this file is byte-for-byte what runs in both places.
 * D3 is imported from an ESM CDN build -- the only runtime dependency.
 *
...
class Plane {
  ...
  renderGrid() {
    ...
    const xGridAxis = d3.axisBottom(this.xScale).tickSize(-this.innerHeight).tickFormat('');
    if (this.opts.xTicks) xGridAxis.tickValues(this.opts.xTicks);
    xGridSel.attr('transform', `translate(0,${this.innerHeight})`).call(xGridAxis);

    let yGridSel = this.gridGroup.selectAll('.y-grid').data([null]);
    yGridSel = yGridSel.enter().append('g').attr('class', 'y-grid').merge(yGridSel);
    const yGridAxis = d3.axisLeft(this.yScale).tickSize(-this.innerWidth).tickFormat('');
    if (this.opts.yTicks) yGridAxis.tickValues(this.opts.yTicks);
    yGridSel.call(yGridAxis);
  }
  ...
  render() {
    ...
    if (showAxes) {
      ...
      // xTicks/yTicks: explicit tick positions (e.g. [0, 1] for a truth-table
      // plane), replacing d3's automatic ticks on both the axes and the grid.
      const xAxis = d3.axisBottom(this.xScale);
      if (this.opts.xTicks) xAxis.tickValues(this.opts.xTicks);
      const yAxis = d3.axisLeft(this.yScale);
      if (this.opts.yTicks) yAxis.tickValues(this.opts.yTicks);
...
class DiagramCore {
  ...
  _renderNodes() {
    ...
    merged.each(function (d) {
      ...
      // Multi-line labels: '\n'-separated, each line its own tspan, all
      // offset via dy only (never an absolute y on a tspan -- Safari/
      // WebKit doesn't apply an inherited dominant-baseline to a tspan
      // that sets its own x/y, silently falling back to alphabetic, so
      // every tspan below also repeats dominant-baseline explicitly
      // rather than relying on inheriting it from the parent <text>;
      // confirmed by measuring getBBox() centering in both Chromium
      // and WebKit -- Chromium tolerated the inherited version fine,
      // only WebKit needed this).
      //
      // 'below'/'above' stack away from a fixed edge, so the first
      // line anchors at yOff (dy: null) and later lines offset by a
      // plain positive dy from there. 'middle' instead centers the
      // whole block on the point -- anchoring the first line at yOff
      // the same way would leave the block's visual center drifting
      // off the point as lines are added, so the first line's dy is
      // pulled up by half the block's total height first.
      const lines = String(d.text || '').split('\n');
      const lineDir = placement === 'above' ? -1 : 1;
      const lineHeightEm = placement === 'middle' ? 1 : 1.2;
      const tspanSel = labelSel.selectAll('tspan').data(lines);
      tspanSel.exit().remove();
      tspanSel
        .enter()
        .append('tspan')
        .merge(tspanSel)
        ...
        .attr('dy', (line, i) => {
          if (placement === 'middle') {
            ...
            return i === 0 ? `${-((n - 1) / 2) * lineHeightEm}em` : `${lineHeightEm}em`;
          }
          return i === 0 ? null : `${lineHeightEm * lineDir}em`;
        })
        .text((line) => line);
    });
        ...
        if (d instanceof Line && d.extent === 'infinite') {
          ...
          if (Math.hypot(dir.x, dir.y) > 1e-6) {
            ...
            if (clipped) {
              drawA = clipped.a;
              drawB = clipped.b;
              // The label goes on the segment actually visible in the
              // plane box, not the two endpoints -- those are often
              // just whatever arbitrary points fixed the line's
              // direction (e.g. an axis intercept far from center) and
              // can sit well outside the plane, or off screen entirely.
              midX = (drawA.x + drawB.x) / 2;
              midY = (drawA.y + drawB.y) / 2;
            }
          }
        }
  ...
  _wireCanvasInteractions() {
    ...
    d3.select(window).on('keydown.diagram-edit', (event) => {
...
export function Diagram(container, opts) {
...
class ControlsPanel {
  ...
  slider(opts) {
    ...
    const valueLabel = header
      .append('span')
      .attr('class', 'controls-value')
      .text(`= ${formatControlValue(initial)}`);

    const showBounds = opts.showBounds !== false;
    const sliderRow = row.append('div').attr('class', 'controls-slider-row');
    if (showBounds) sliderRow.append('span').attr('class', 'controls-slider-bound').text(formatControlValue(min));

    const input = sliderRow
      .append('input')
      .attr('type', 'range')
    ...
    if (opts.width) input.style('flex', '0 0 auto').style('width', `${opts.width}px`);

    if (showBounds) sliderRow.append('span').attr('class', 'controls-slider-bound').text(formatControlValue(max));
  ...
  // Two narrow sliders sharing one row -- for when a node's params are
  // naturally paired (e.g. the two incoming weights of a unit) and
  // stacking them as separate full-width slider() rows costs more
  // height than the panel can spare. Same bind/state wiring as
  // slider(), just laid out two-up; bounds are never shown (there's no
  // room), values always are.
  sliderPair(a, b) {
    const row = this.panel.append('div').attr('class', 'controls-row');
    const pairRow = row.append('div').attr('class', 'controls-pair-row');
    this._miniSlider(pairRow, a);
    this._miniSlider(pairRow, b);
    return this;
  }

  _miniSlider(container, opts) {
    opts = opts || {};
    const bind = opts.bind;
    const min = opts.min != null ? opts.min : 0;
    const max = opts.max != null ? opts.max : 1;
    ...
    const col = container.append('div').attr('class', 'controls-pair-item');
    const header = col.append('div').attr('class', 'controls-row-header');
    header.append('span').attr('class', 'controls-label controls-label-sm').text(opts.label || bind || '');
    const valueLabel = header
      .append('span')
      .attr('class', 'controls-value controls-value-sm')
      .text(`= ${formatControlValue(initial)}`);

    const sliderRow = col.append('div').attr('class', 'controls-slider-row');
    const input = sliderRow
      .append('input')
      .attr('type', 'range')
      .attr('class', 'controls-slider')
...
export function Controls(container, state) {
