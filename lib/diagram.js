...
class DiagramCore {
  ...
  _renderNodes() {
    ...
    merged.each(function (d) {
      ...
      // Multi-line labels: '\n'-separated, each line its own tspan
      // stacked in whichever direction the label already grows in
      // (down for 'below'/'middle', up for 'above' so extra lines
      // never grow back toward the node).
      ...
      tspanSel
        ...
        .merge(tspanSel)
        .attr('x', 0)
        .attr('dy', (line, i) => (i === 0 ? null : `${1.2 * lineDir}em`))
        .text((line) => line);
    });
...
class ControlsPanel {
  ...
  slider(opts) {
    ...
    sliderRow.append('span').attr('class', 'controls-slider-bound').text(formatControlValue(min));
    ...
    sliderRow.append('span').attr('class', 'controls-slider-bound').text(formatControlValue(max));
